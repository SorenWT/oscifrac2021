...
save('Fig4b_param_tc_effsize.mat','p')
savefig('Fig4b_param_tc_effsize.fig'); export_fig('Fig4b_param_tc_prcchange.png','-m5')
...
effsizefun = @(post,bl,rest,mask) nansum(nansum(abs(nanmean((log10(post)-nanmean(log10(bl),3))./nanstd(log10(rest),[],3),1)).*mean(mask,1),3),2);
...
effsizetc.int = nanmean((intdata.post-nanmean(intdata.bl,3))
...
for i = 1:length(fields)
   ...
   rest1 = squeeze(10.^tresdata.grpdata(:,:,i,:));
   ...
   for ii = 1:(i-1)
       ...
       rest2 = squeeze(10.^tresdata.grpdata(:,:,i,:));
